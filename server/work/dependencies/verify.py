...
from work.exceptions.base import AppException
from work.front.config import FrontConfig
from work.http_base import HttpResp
from work.utils.redis import RedisUtil
...
async def verify_token(request: Request):
    ...
    from work.utils.redis import RedisUtil
    from work.exceptions.base import AppException
    from work.admin.config import AdminConfig
    from work.admin.service.system.auth_admin import SystemAuthAdminService
    from work.admin.service.system.auth_perm import SystemAuthPermService

    # 路由转权限
    auths = request.url.path.replace('/platform/', '').replace('/', ':')
    ...
    for role_id in role_ids:
        ...
        menus_str = await RedisUtil.hget(AdminConfig.backstage_roles_key, role_id)
        print(AdminConfig.backstage_roles_key)
        print(menus_str)
        menus.extend(menus_str.split(','))
    # 验证是否有权限操作
...
async def front_login_verify(request: Request):
    ...
    from work.front.service.login import LoginService
